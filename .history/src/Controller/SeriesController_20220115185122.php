...
class SeriesController extends AbstractController
{
    ...
    /**
     * @Route("/croissantes", name="tri_croissant", methods={"GET"})
     */
    public function croissant(EntityManagerInterface $entityManager, PaginatorInterface $paginator, Request $request, calculMoyenne $calculMoyenne): Response
    {
        $calculMoyenne->setMoyenne($entityManager);
        
        $queryBuilder=$this->createQueryBuilder('s');
        $queryBuilder->where('s.noteUSer = :user_id')
     ->setParameter('user_id', $user_id)
     <strong>->andWhere('id = (select max(r2.id) from reponse r2 where r2.problematique_id = r.problematique_id)')</strong>
     ->OrderBy('r.date_creation_reponse','DESC');
     return $queryBuilder->getQuery()->getResult();
        ### gere la recherche
        $repository = $entityManager->getRepository(Series::class);
        
        $series = $paginator->paginate(
            $repository->findNotesCroissantes(), /* query NOT result */
            $request->query->getInt('page', 1), /*page number*/
            12 /*limit per page*/
        );
